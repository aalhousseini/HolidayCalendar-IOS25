...
//                    UserDefaults.standard.set(true, forKey: "firstCalendar")
//                } label: {
//                    Text("Logout")
        ...
        NavigationView {
            ScrollView {
                VStack(spacing: 30) {
                    ...
                    // Time Left to Open Section
                    ZStack {
                        ForEach((0..<3).reversed(), id: \.self) { index in
                            RoundedRectangle(cornerRadius: 15)
                                .fill(index == 0 ? .black : .gray) // All cards black
                                .frame(width: 300, height: 120)
                                .shadow(color: Color.gray.opacity(0.5), radius: 8, x: 5, y: 5)
                                .offset(x: CGFloat(index * 10), y: CGFloat(index * 10)) // Offset for stacking
                        }
                        Text("Time left to open: \(timeLeftToOpen)")
                            .font(.custom("Georgia", size: 15))
                            .foregroundColor(.white)
                ...
                .padding(.bottom, 40) // To avoid overlapping with the TabBar
            }
            .navigationBarTitleDisplayMode(.inline)
            .toolbar {
                ToolbarItem(placement: .navigationBarLeading) {
